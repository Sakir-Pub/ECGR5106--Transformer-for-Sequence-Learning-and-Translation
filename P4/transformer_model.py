...
class TransformerEncoder(nn.Module):
    """
    Transformer Encoder for the French to English translation model.
    """
...
class TransformerDecoder(nn.Module):
    """
    Transformer Decoder for the French to English translation model.
    """
...
class TransformerSeq2Seq(nn.Module):
    """
    Full Transformer Sequence-to-Sequence model for French to English translation
    """
...
def translate_sentence_transformer(model, sentence, french_vocab, english_vocab, 
                                  device, max_length=100):
    """
    Translate a sentence from French to English using the transformer model.
    
    Args:
        model: Trained transformer model
        sentence: French sentence to translate
    ...
    if isinstance(english_vocab, dict) and 'word2idx' in english_vocab:
        ...
        eng_idx2word = english_vocab['idx2word']
    ...
    tokens = [fr_word2idx.get(word, fr_word2idx['<UNK>']) for word in sentence.lower().split()]
    tokens.append(fr_word2idx['<EOS>'])
    ...
    trg_tokens = [eng_idx2word[i] for i in trg_indexes]
