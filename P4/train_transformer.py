...
def print_example_translations(model, model_type, data_loader, french_vocab, english_vocab, device, num_examples=5):
    """Print example translations from the original dataset."""
    print(f"\n----- Example Translations ({model_type}) -----")
    
    if hasattr(data_loader, 'french_sentences') and hasattr(data_loader, 'english_sentences'):
        # Get random indices
        indices = random.sample(range(len(data_loader.french_sentences)), min(num_examples, len(data_loader.french_sentences)))
        
        for idx in indices:
            fr_sentence = data_loader.french_sentences[idx]
            actual_eng_sentence = data_loader.english_sentences[idx]
            
            # Get model translation
            if model_type == 'rnn':
                translated_sentence = translate_sentence(model, fr_sentence, french_vocab, english_vocab, device)
            else:  # transformer
                translated_sentence = translate_sentence_transformer(model, fr_sentence, french_vocab, english_vocab, device)
            
            print(f"French: {fr_sentence}")
            print(f"Actual English: {actual_eng_sentence}")
            print(f"Predicted English: {translated_sentence}")
            print("-" * 50)
    else:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train a Transformer or RNN model for French to English translation')
...
def train_model(model, data_loader, model_type, args, config_suffix=""):
    ...
    pad_idx = special_tokens['english']['pad']
        ...
        if val_accuracy > best_val_accuracy:
            ...
            if model_type == 'transformer':
                save_dict = {
                    ...
                    'input_dim': len(data_loader.french_vocab),
                    'output_dim': len(data_loader.english_vocab),
...
def compare_transformer_configs(args, data_loader):
    """Train and compare all transformer configurations as per assignment requirements."""
    # Define the configurations to test (1, 2, 4 layers with 2, 4 heads)
    layer_options = [1, 2, 4]
    head_options = [2, 4]
    ...
    for num_layers in layer_options:
        for num_heads in head_options:
            ...
            french_vocab_size, english_vocab_size = data_loader.get_vocab_sizes()
            special_tokens = data_loader.get_special_tokens()
            pad_idx = special_tokens['english']['pad']
            ...
            model = init_transformer_model(
                ...
                dim_feedforward=args.dim_feedforward,
                dropout=args.dropout,
                src_pad_idx=special_tokens['french']['pad'],
                trg_pad_idx=pad_idx,
                device=device
            )
...
def compare_with_rnn_models(args, data_loader, transformer_results):
    """Compare the best transformer model with RNN-based models."""
    comparison_results = {}
    
    # 1. Train standard RNN
    print(f"\n{'='*80}")
    print(f"Training RNN model for French-to-English translation")
    print(f"{'='*80}\n")
    ...
    # Train RNN model
    _, rnn_accuracy = train_model(rnn_model, data_loader, 'rnn', args)
    comparison_results['RNN'] = rnn_accuracy
    
    # Find best transformer config and its accuracy
    best_config = max(transformer_results.items(), key=lambda x: x[1])[0]
    best_transformer_accuracy = transformer_results[best_config]
    comparison_results['Best Transformer' + best_config] = best_transformer_accuracy
    
    # Print comparison
    print("\n\nModel Comparison (French-to-English):")
    print("=" * 50)
    for model_name, accuracy in sorted(comparison_results.items(), key=lambda x: x[1], reverse=True):
        print(f"{model_name}: Accuracy = {accuracy:.4f}")
    
    # Save comparison results
    with open(os.path.join(args.metrics_dir, 'fr2en_model_comparison.json'), 'w') as f:
        json.dump(comparison_results, f, indent=4)
    
    return comparison_results


def compare_translation_directions(args, fr2en_results):
    """Compare French-to-English with English-to-French translation performance."""
    # Load or calculate English-to-French results
    # This function assumes you have previously run the original English-to-French
    # experiments and saved the results
    
    try:
        # Try to load the previous English-to-French results
        with open(os.path.join(args.metrics_dir, 'en2fr_model_comparison.json'), 'r') as f:
            en2fr_results = json.load(f)
        
        print("\nComparing translation directions:")
        print("=" * 50)
        print("French-to-English models:")
        for model, accuracy in sorted(fr2en_results.items(), key=lambda x: x[1], reverse=True):
            print(f"  {model}: Accuracy = {accuracy:.4f}")
        
        print("\nEnglish-to-French models:")
        for model, accuracy in sorted(en2fr_results.items(), key=lambda x: x[1], reverse=True):
            print(f"  {model}: Accuracy = {accuracy:.4f}")
        
        # Compare best models from each direction
        best_fr2en = max(fr2en_results.items(), key=lambda x: x[1])
        best_en2fr = max(en2fr_results.items(), key=lambda x: x[1])
        
        print("\nBest French-to-English model:")
        print(f"  {best_fr2en[0]}: Accuracy = {best_fr2en[1]:.4f}")
        
        print("Best English-to-French model:")
        print(f"  {best_en2fr[0]}: Accuracy = {best_en2fr[1]:.4f}")
        
        # Determine which direction seems more effective
        if best_fr2en[1] > best_en2fr[1]:
            print("\nFrench-to-English translation appears to be more effective.")
        elif best_fr2en[1] < best_en2fr[1]:
            print("\nEnglish-to-French translation appears to be more effective.")
        else:
            print("\nBoth translation directions seem equally effective.")
        
        # Save comparison to file
        direction_comparison = {
            'French-to-English': fr2en_results,
            'English-to-French': en2fr_results,
            'Best French-to-English': {best_fr2en[0]: best_fr2en[1]},
            'Best English-to-French': {best_en2fr[0]: best_en2fr[1]}
        }
        
        with open(os.path.join(args.metrics_dir, 'translation_direction_comparison.json'), 'w') as f:
            json.dump(direction_comparison, f, indent=4)
        
    except FileNotFoundError:
        print("\nCouldn't find English-to-French results for comparison.")
        print("Please run the English-to-French experiments first to enable direction comparison.")
        
        # Still save the French-to-English results for future comparison
        with open(os.path.join(args.metrics_dir, 'fr2en_model_comparison.json'), 'w') as f:
            json.dump(fr2en_results, f, indent=4)
...
if __name__ == "__main__":
    ...
    if args.compare_all_configs:
        # First, compare all transformer configurations (1, 2, 4 layers with 2, 4 heads)
        transformer_results = compare_transformer_configs(args, data_loader)
        
        # Then compare with RNN models
        fr2en_results = compare_with_rnn_models(args, data_loader, transformer_results)
        
        # Finally, compare translation directions (if possible)
        compare_translation_directions(args, fr2en_results)
