...
class DataAugmenter:
    def __init__(self, english_sentences, french_sentences, seed=42):
        """
        Initialize data augmenter for English-French translation pairs
    ...
    def generate_augmented_data(self, augment_ratio=0.5):
        ...
        Returns:
            Tuple of (english_sentences, french_sentences) with augmentations
        """
        num_augmented = int(len(self.english_sentences) * augment_ratio)
        if num_augmented == 0:
            return self.english_sentences, self.french_sentences
        
        # Randomly select indices to augment
        indices_to_augment = random.sample(range(len(self.english_sentences)), num_augmented)
...
class DatasetLoader:
    ...
    def _load_data(self):
        ...
        return list(zip(english_indices, french_indices))
    ...
    def _create_augmented_data(self):
        ...
        aug_english, aug_french = augmenter.generate_augmented_data(self.augment_ratio)
        ...
        return list(zip(aug_english_indices, aug_french_indices))
    ...
    def _create_dataloader(self, data, shuffle=True):
        ...
        english_data = [item[0] for item in data]
        french_data = [item[1] for item in data]
        ...
        def collate_fn(batch):
            ...
            french_input = [torch.tensor([self.french_vocab.word2idx["<SOS>"]] + seq.tolist()[:-1])  # Exclude EOS
                           for seq in french_sequences]
            ...
            padded_french_input = pad_sequence(french_input, batch_first=True, 
                                              padding_value=self.french_vocab.word2idx["<PAD>"])
            
            return {
                "encoder_inputs": padded_english,
                "decoder_inputs": padded_french_input,
                "decoder_targets": padded_french
            }
    ...
    def get_vocab_sizes(self):
        ...
        return len(self.english_vocab), len(self.french_vocab)
    
    def get_max_lengths(self):
        """Return maximum sequence lengths"""
        return self.max_english_length, self.max_french_length
...
if __name__ == "__main__":
    ...
    for batch in loader.train_loader:
        ...
        # Display a sample
        sample_idx = 0
        english_indices = batch['encoder_inputs'][sample_idx].tolist()
        french_indices = batch['decoder_targets'][sample_idx].tolist()
